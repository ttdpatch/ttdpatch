...
var signaloffsets
	// the (ever so slightly adjusted) original offsets for signals on the left side:
	db 0x8+1,0xE,0x1,0x9-3,0x1,0x3+3-3,0xB-2+3,0xE,0x3,0xD,0xB,0x4	// X offsets
	db 0x5+1,0x1,0xE,0xB-2,0x0,0xA-4+3,0x4+4-3,0xE,0x4,0xB,0x3,0xD	// Y offsets
	// and now the offsets for signals on the right side:
	db 0xE,0xA-1,0x5+1,0x1,0xA-4+3,0x0,0xE,0x4+4-3,0xD,0x3,0xB,0x4	// X offsets
	db 0x1,0x8-2,0x7+2,0xE,0x3+3-3,0x1,0xE,0xB-2+3,0x4,0xB,0xD,0x3	// Y offsets
//(Remove the +3/-3 for EW track for centrally positioned and equally signals signals)
