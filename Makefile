...
host/%.o.d:
	${_E} [HOSTCC DEP] $@
	${_C}$(HOSTCC) -c $(CFLAGS) $(foreach DEF,$(WINDEFS),-D$(DEF)) -MM -MG -MF $@ -MT ${subst .d,,$@} $*.c -o /dev/null
%.o.d:
	${_E} [CC DEP] $@
	${_C}$(CC) -c $(CFLAGS) $(foreach DEF,$(WINDEFS),-D$(DEF)) -MM -MG -MF $@ -MT ${subst .d,,$@} $*.c -o /dev/null
%.obj.d : %.c
	${_E} [CC DEP] $@
	${_C}$(CC) -c $(CFLAGS) $(foreach DEF,$(DOSDEFS),-D$(DEF)) -MM -MG -MF $@ -MT ${subst .d,,$@} $< -o /dev/null
